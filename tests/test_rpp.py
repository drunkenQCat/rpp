...
def test_scanner():
    from rpp.rpp import tokenize

    # Test the new tokenizer
    tokens = list(tokenize('<REAPER - 5 05 0.5 "4.32"\n>'))
    assert [tok.value for tok in tokens] == [
        "<",
        "REAPER",
        "-",
        "5",
    ...
    tokens = list(tokenize('<"REAPER"\n>'))
    # Note: the new tokenizer returns quoted strings with quotes stripped
    assert [tok.value for tok in tokens] == [
        "<",
        "REAPER",
        "\n",
...
def test_tostr():
    ...
    assert (
        encode_value("{010F6508-D16E-4DA0-BE44-E8F3C39D6314}")
        == "{010F6508-D16E-4DA0-BE44-E8F3C39D6314}"
    )
